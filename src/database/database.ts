...
export function initDatabase(): Promise<void> {
  if (!initPromise) {
    initPromise = executeInitDatabase().catch((err) => {
      initPromise = null;
      throw err;
    });
  }
...
async function executeInitDatabase() {
  await db.execAsync(`
  ...
  await db.execAsync(`
    DELETE FROM exercicios WHERE id NOT IN (
      SELECT MIN(id) FROM exercicios GROUP BY nome
    );
  `);
  ...
  if (!existing || existing.count === 0) {
    ...
    await inserirCatalogoExercicios();
  }
}

export async function inserirCatalogoExercicios() {
  await db.runAsync(`
    INSERT OR IGNORE INTO exercicios (nome, descricao, grupo_muscular) VALUES
    -- Peitoral
...
export async function resetarCatalogoPadrao() {
  await db.execAsync(`
    DELETE FROM exercicios WHERE id NOT IN (
      SELECT exercicio_id FROM exercicio_treino
    );
  `);
  await inserirCatalogoExercicios();
  await db.execAsync(`
